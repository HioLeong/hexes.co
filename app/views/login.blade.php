<html><head>
	<meta http-equiv="Content-Type" content="text/html; charset=utf-8">
	<meta name="viewport" content="width=device-width, initial-scale = 1.0, user-scalable = no">
	<title>Hexes</title>
	<link rel="stylesheet" href="css/style.css" type="text/css" media="screen">
	<link rel="stylesheet" href="css/jquery-ui-1.10.1.custom.css" type="text/css" media="screen">
	<link rel="stylesheet" href="css/normalize.css" type="text/css" media="screen">
	<link rel="stylesheet" href="css/grid.css" type="text/css" media="screen">
	<link href="css/prettyPhoto.css" rel="stylesheet" type="text/css">
	
	<link href="http://fonts.googleapis.com/css?family=PT+Sans:400,400italic,700,700italic" rel="stylesheet" type="text/css">
	
	<script src="http://ajax.googleapis.com/ajax/libs/jquery/1.10.2/jquery.min.js"></script>
	<script type="text/javascript" src="js/jquery.easing.1.3.js"></script>
	<script type="text/javascript" src="js/jquery.prettyPhoto.js"></script>
	<script src="js/jquery-ui-1.10.4.custom.min.js"></script><style type="text/css"></style>
	<script src="js/select.js"></script> 
	<script type="text/javascript" src="js/js.js"></script>
	<script type="text/javascript" src="js/jquery.stellar.min.js"></script>
	<script type="text/javascript" src="js/waypoints.min.js"></script>

	
	<script type="text/javascript" src="js/angular.js"></script>
    <script type="text/javascript" src="js/hexApp.js"></script>

	<!--[if lt IE 9]>
		<script src="js/html5shiv.js"></script>
		<script src="js/respond.src.js"></script>
	<![endif]-->
	
	<script type="text/javascript">
	</script>
	
	<script type="text/javascript">//$($.date_input.initialize);</script>
	
<style type="text/css">
.gpitPhotoStorage {
    height:0px;
    width:0px;
    position:absolute;
    top:-100px;
    left:-100px;
    overflow:hidden;
    visibility:hidden;
}

.gpitphotoViewer {
    position:absolute; 
    overflow:hidden; 
    font-size:0;
    -moz-box-shadow:0px 0px 10px rgba(0,0,0,1); 
    -webkit-box-shadow:0px 0px 10px rgba(0,0,0,1); 
    box-shadow:0px 0px 10px rgba(0,0,0,1); 
    min-height:50px; 
    min-width:50px;
    padding:5px; 
    pointer-events:none; 
    background:#fff url('data:image/gif;base64,') no-repeat center center; 
    z-index:99999;
    opacity:0;
}

.gpitImage {
    position:relative;
    z-index:999;
}

.gpitmark {
    background:url('data:image/png;base64,') repeat-x left top;
    height:26px;
    width:auto;
    margin-top:1px;
    overflow:hidden;
    display:none;
}

.gpitmark .gpitlogo {
    background:url('data:image/png;base64,') no-repeat left top;
    width:34px;
    float:left;
    height:26px;
    overflow:hidden;
}

.gpitmark .gpitmarktext {
    color:gray;
    margin-left:4px;
    font-size:11px;
    line-height:28px;
    float:left;
    height:26px;
    overflow:hidden;
}

.gpitstatus {
    border:3px solid #ecf0f5;
    -webkit-border-radius: 5px;
    -moz-border-radius: 5px;
    border-radius: 5px;
    background:#fff url('data:image/png;base64,') repeat-x left top;
    height:30px;
    padding:3px 10px;
    overflow:hidden;
    position:fixed;
    top:10px;
    right:10px;
    z-index:999999;
    display:none;    
}

.gpitstatus .gpitlogo {
    background:url('data:image/png;base64,') no-repeat left top;
    height:30px;
    width:96px;
    float:left;
    overflow:hidden;
}

.gpitstatus .gpitstatustxt {
    color:gray;
    float:left;
    font-size:12px;
    line-height:32px;
}

*[class*="Image"]:after, [class*="image"]:after, *[class*="Photo"]:after, *[class*="photo"]:after {
    pointer-events:none !important;
}</style></head>

<body crossrider_data_store_temp="{}" ng-app="hexApp">

<div class="menu">	
	<div class="container clearfix">

		<div id="logo" class="fleft">
			<a><img src="images/hexlogo2.png" height="74" ></a>
		</div>
		<div id="nav" class="fright" ng-controller="LoginCtrl">
                <form ng-controller="LoginCtrl" ng-submit="fun()">
                    <div class="navigation">
                        <input id="username" type="text" placeholder="email" ng-model='username'/>
                        <input id="password" type="password" placeholder="password" ng-model='password'/>
                        <input style="visibility:hidden" type="submit"/>
                        </div>
                </form>
     
          <!--<form class="navbar-form navbar-right" role="form">
            <div class="form-group">
              <input type="text" placeholder="Email" id="emailLogin" class="form-control">
            </div>
            <div class="form-group">
              <input type="password" placeholder="Password" id="passLogin" class="form-control">
            </div>
            <img id="hexlogin" class="hexLoginClass" src="images/signin.png">
          </form>
				<li data-slide="1" class="active">Home</li>
				<li data-slide="2">About</li>
				<li data-slide="4">Services</li>
				<li data-slide="6">Gallery</li>
				<li data-slide="10">Contact</li>
				<div class="clear"></div>-->
			</ul>
		</div>
	
	</div>
</div>


<div class="slide" id="slide1" data-slide="1" data-stellar-background-ratio="0.5" style="background-position: 0px 0px;">
	<div class="padding_slide1">
		<div class="container clearfix">
			<div id="content" class="grid_12">
				<div class="filtr_bg">
                <img src="images/hexlogo.png" height="128" width="128" style="margin-bottom: 20px;">
					<p>Modern Socialite</p>
					<h1>welcome to HEXES</h1>
					<h2>we do our utmost to leave only the best impressions<br>after your stay at our hotel</h2>
				</div>
				<a class="button" title="" data-slide="8">Register</a>
			</div>
		</div>
	</div>
</div>



<div class="slide" id="slide8" data-slide="8" data-stellar-background-ratio="0.5" style="background-size: 100% 664px; background-position: 0px 432px;">
	<div class="effect_2 padding_slide8">
		<div class="container clearfix">
			
			<div id="content" class="grid_12">
				<div id="content" class="grid_12">
					<h1 style="color:#000;">Register</h1>
				</div>
				<h2 style="color:#000;">When would you like to stay at HotelName?</h2>
				<div class="order_block">
					
                    <form class="form" method="post" action="main.html">
    <input type="email" class="email" placeholder="Email">
    <div>
      <p class="email-help regText">Please enter your current email address for verification.</p>
    </div>
    <input type="password" class="password" placeholder="Password">
     <div>
      <!--<p class="password-help">Please enter your current email address.</p>-->
    </div>
    <input type="password" class="confirmPassword" id="cp" placeholder="Confirm Password">
     <div>
      <p class="password-help regText">Please confirm your password.</p>
    </div>
    <input type="submit" class="submit" value="Register">
  </form>
					<!--<form method="post" action="">
						<label>Room:</label>
						<select name="fancySelect" class="makeMeFancy" style="display: none;">
							<option value="0" selected="selected" data-skip="1">Select Room Type</option>
							<option value="1" data-html-text="Lorem Ipsum&lt;i&gt;&lt;/i&gt;">Lorem Ipsum</option>
							<option value="2" data-html-text="Dolor sit amet&lt;i&gt;&lt;/i&gt;">Dolor sit amet</option>
							<option value="3" data-html-text="Sed ut perspiciatis&lt;i&gt;&lt;/i&gt;">Sed ut perspiciatis</option>
							<option value="4" data-html-text="Nemo enim ipsam voluptatem&lt;i&gt;&lt;/i&gt;">Nemo enim ipsam voluptatem</option>
						</select><div class="tzSelect" style="width: 192px;"><div class="selectBox"></div><ul class="dropDown" style="display: none;"><li><img src="undefined"><span>Lorem Ipsum<i></i></span></li><li><img src="undefined"><span>Dolor sit amet<i></i></span></li><li><img src="undefined"><span>Sed ut perspiciatis<i></i></span></li><li><img src="undefined"><span>Nemo enim ipsam voluptatem<i></i></span></li></ul></div>
						<label>From:</label><input type="text" id="datepicker" class="hasDatepicker">
					<label>To:</label><input type="text" id="datepicker2" class="hasDatepicker">
					<label>Email:</label><input class="email_form" type="text" name="text">					
					<input type="submit" class="form_btn contact_btn send_btn" value="Book">
					</form>-->
					
				</div>
			</div>
		</div>
	</div>
</div>


<!--
<div class="slide" id="slide9" data-slide="9" data-stellar-background-ratio="0.5" style="background-position: 0px 764px;">
	<div class="container clearfix">
	
		<div id="content" class="grid_12">
			<p>Pellentesque habitant morbi tristique senectus et netus et malesuada fames ac</p>
		</div>
	
	</div>
</div>



<div class="slide" id="slide2" data-slide="2" data-stellar-background-ratio="0.5" style="background-size: 100% 395px; background-position: 0px 905.5px;">
	<div class="effect_2 padding_slide2">
		<div class="container clearfix">
			
			<div id="content" class="grid_12">
				<h1>About</h1>
				<h2>Cras urna leo, fringilla nec aliquam ac, varius in enim. Maecenas non felis</h2>
			</div>
			
		</div>
	</div>
</div>



<div class="slide" id="slide3" data-slide="3" data-stellar-background-ratio="0.5" style="background-position: 0px -1103px;">
	<div class="container clearfix">

		<div id="content" class="grid_12">
			<img class="about_img" src="images/about_img1.jpg" alt="">
			<h4>Our awesomeness</h4>
			<p>Ut pretium condimentum diam quis gravida. Donec odio justo, mollis eget tristique vel, feugiat ut lectus. Donec consectetur leo sed nibh tempus eget consequat quam interdum. Morbi egestas volutpat metus, nec ullamcorper eros pharetra quis. Praesent ante nisi, pellentesque ac scelerisque id, sodales ac erat. In pretium, magna consectetur malesuada bibendum, neque elit facilisis metus, sit amet convallis est tortor vel urna. Sed consequat nisl quis lorem varius feugiat porta lorem imperdiet. Lorem ipsum dolor sit amet, consectetur adipiscing elit. Duis quis sollicitudin risus. Quisque at quam magna. Ut facilisis sodales pellentesque. Nullam ultrices ullamcorper suscipit.</p>
			<p>Praesent leo est, pharetra sed varius ut, pulvinar ac arcu. Maecenas et purus ut dolor mattis suscipit. Pellentesque accumsan risus a magna iaculis vel imperdiet massa luctus. Phasellus mattis neque nec mi pulvinar eget molestie nibh lacinia. Pellentesque orci enim, molestie quis gravida id, feugiat vitae dui. Praesent eu tincidunt sem. Etiam nunc nisi, convallis sed imperdiet sit amet, ultricies eget est. Vivamus feugiat tellus in diam laoreet blandit. Nullam at dui sit amet nibh iaculis consectetur. Proin erat, sit amet euismod nibh. Morbi enim turpis, porttitor eget lacinia in, facilisis id ligula. Praesent id tellus metus, vitae iaculis quam. Class aptent taciti litora torquent per conubia nostra, per inceptos himenaeos. Vestibulum quis risus adipiscing purus dictum elementum.</p>
		</div>

	</div>
</div>



<div class="slide" id="slide4" data-slide="4" data-stellar-background-ratio="0.5" style="background-size: 100% 395px; background-position: 0px 1339.5px;">
	<div class="effect_2 padding_slide4">
		<div class="container clearfix">
			
			<div id="content" class="grid_12">
				<h1><span>Services</span></h1>
				<h2>Cras urna leo, fringilla nec aliquam ac, varius in enim. Maecenas non felis</h2>			
			</div>

		</div>
	</div>
</div>
-->

<div class="slide" id="slide5" data-slide="5" data-stellar-background-ratio="0.5" style="background-position: 0px -1537px;">
	<div class="container clearfix">

		<div class="grid_4">
			<a class="serv_block" href="javascript:void(0);" alt="">
				<div class="icon_block1"></div>
				<p>Connect</p>
				Mauris sit amet massa enim. Vivamus scelerisque feugiat ornare. Nulla quis nulla lectus. Curabitur
			</a>
		</div>
		<div class="grid_4">
			<a class="serv_block" href="javascript:void(0);" alt="">
				<div class="icon_block2"></div>
				<p>Share</p>
				Vivamus rhoncus facilisis orci et sagittis. Aliquam mollis commodo auctor. Aenean id neque libero
			</a>
		</div>
		<div class="grid_4 omega">
			<a class="serv_block" href="javascript:void(0);" alt="">
				<div class="icon_block3"></div>
				<p>Hexes</p>
				Donec in ligula eget quam elementum consectetur id vel ante. Donec tristique, mi ac faucibus
			</a>
		</div>
	
	</div>
</div>



<div class="slide" id="slide6" data-slide="6" data-stellar-background-ratio="0.5" style="background-size: 100% 395px; background-position: 0px 1790px;">
	<div class="effect_2 padding_slide6">
		<div class="container clearfix">
			
			<div id="content" class="grid_12">
				<h1><span>Gallery</span></h1>
				<h2>Cras urna leo, fringilla nec aliquam ac, varius in enim. Maecenas non felis</h2>
			</div>
			<div class="clear"></div>

		</div>
	</div>
</div>


<div class="slide" id="slide7" data-slide="7" data-stellar-background-ratio="0.5" style="background-position: 0px -1987.5px;">
	<div class="container clearfix">
	
		<div class="overflow_hidden">
			<!-- gallery_block -->
			<ul class="gallery-list">                                  
				<li class="grid_3">
					<div class="hover_img">
						<img src="images/gallery/1.jpg" alt="">
						<div class="item_description">
							<p>Praesent convallis</p>
							Cras quis arcu lorem. Aenean feugiat auctor risus, eget sodales turpis fringilla id.
							<span class="portfolio_zoom"><a href="images/gallery/full/1.jpg" rel="prettyPhoto[portfolio1]"></a></span>
							<span class="portfolio_link"><a href="javascript:void(0);"></a></span>
						</div>
						<p>Praesent convallis</p>
					</div>
				</li>
				<li class="grid_3">
					<div class="hover_img">
						<img src="images/gallery/2.jpg" alt="">
						<div class="item_description">
							<p>Suspendisse varius leo</p>
							Cras quis arcu lorem. Aenean feugiat auctor risus, eget sodales turpis fringilla id.
							<span class="portfolio_zoom"><a href="images/gallery/full/2.jpg" rel="prettyPhoto[portfolio1]"></a></span>
							<span class="portfolio_link"><a href="javascript:void(0);"></a></span>
						</div>
						<p>Suspendisse varius leo</p>
					</div>
				</li>
				<li class="grid_3">
					<div class="hover_img">
						<img src="images/gallery/3.jpg" alt="">
						<div class="item_description">
							<p>Vivamus in sapien nec</p>
							Cras quis arcu lorem. Aenean feugiat auctor risus, eget sodales turpis fringilla id.
							<span class="portfolio_zoom"><a href="images/gallery/full/3.jpg" rel="prettyPhoto[portfolio1]"></a></span>
							<span class="portfolio_link"><a href="javascript:void(0);"></a></span>
						</div>
						<p>Vivamus in sapien nec</p>
					</div>
				</li>
				<li class="grid_3">
					<div class="hover_img">
						<img src="images/gallery/4.jpg" alt="">
						<div class="item_description">
							<p>Praesent quam ipsum</p>
							Cras quis arcu lorem. Aenean feugiat auctor risus, eget sodales turpis fringilla id.
							<span class="portfolio_zoom"><a href="images/gallery/full/4.jpg" rel="prettyPhoto[portfolio1]"></a></span>
							<span class="portfolio_link"><a href="javascript:void(0);"></a></span>
						</div>
						<p>Praesent quam ipsum</p>
					</div>
				</li>
				<li class="grid_3">
					<div class="hover_img">
						<img src="images/gallery/5.jpg" alt="">
						<div class="item_description">
							<p>Suspendisse varius leo</p>
							Cras quis arcu lorem. Aenean feugiat auctor risus, eget sodales turpis fringilla id.
							<span class="portfolio_zoom"><a href="images/gallery/full/5.jpg" rel="prettyPhoto[portfolio1]"></a></span>
							<span class="portfolio_link"><a href="javascript:void(0);"></a></span>
						</div>
						<p>Suspendisse varius leo</p>
					</div>
				</li>
				<li class="grid_3">
					<div class="hover_img">
						<img src="images/gallery/6.jpg" alt="">
						<div class="item_description">
							<p>Sed non quam lacus</p>
							Cras quis arcu lorem. Aenean feugiat auctor risus, eget sodales turpis fringilla id.
							<span class="portfolio_zoom"><a href="images/gallery/full/6.jpg" rel="prettyPhoto[portfolio1]"></a></span>
							<span class="portfolio_link"><a href="javascript:void(0);"></a></span>
						</div>
						<p>Sed non quam lacus</p>
					</div>
				</li>
				<li class="grid_3">
					<div class="hover_img">
						<img src="images/gallery/7.jpg" alt="">
						<div class="item_description">
							<p>Mauris nec odio ut lacus</p>
							Cras quis arcu lorem. Aenean feugiat auctor risus, eget sodales turpis fringilla id.
							<span class="portfolio_zoom"><a href="images/gallery/full/7.jpg" rel="prettyPhoto[portfolio1]"></a></span>
							<span class="portfolio_link"><a href="javascript:void(0);"></a></span>
						</div>
						<p>Mauris nec odio ut lacus</p>
					</div>
				</li>
				<li class="grid_3">
					<div class="hover_img">
						<img src="images/gallery/1.jpg" alt="">
						<div class="item_description">
							<p>Curabitur mattis erat</p>
							Cras quis arcu lorem. Aenean feugiat auctor risus, eget sodales turpis fringilla id.
							<span class="portfolio_zoom"><a href="images/gallery/full/1.jpg" rel="prettyPhoto[portfolio1]"></a></span>
							<span class="portfolio_link"><a href="javascript:void(0);"></a></span>
						</div>
						<p>Curabitur mattis erat</p>
					</div>
				</li>
				<div class="clear"></div>
			</ul>   
			<!-- //gallery_block -->   
		</div>
	
	</div>
</div>




<div class="slide" id="slide10" data-slide="10" data-stellar-background-ratio="0.5" style="background-size: 100% 395px; background-position: 0px 2404px;">
	<div class="effect_2 padding_slide10">
		<div class="container clearfix">
			
			<div id="content" class="grid_12">
				<h1><span>Contact</span></h1>
			<h2>Cras urna leo, fringilla nec aliquam ac, varius in enim. Maecenas non felis</h2>
			</div>
			<div class="clear"></div>

		</div>
	</div>
	
</div>


<!--
<div class="slide" id="slide11" data-slide="11" data-stellar-background-ratio="0.5" style="background-position: 0px -2601.5px;">
	<div class="container clearfix">
		
		<div class="grid_3 contact_det_block">
			<p>Quisque imperdiet neque et nunc laoreet nec luctus erat consectetur. Proin dapibus aliquet odio</p>
			<ul class="foot_block_intouch">
				<li class="touch_phone"><p>(+1) 245 5462 252</p></li>
				<li class="touch_clock"><p>Mon - Sat 9:00am - 6:00 pm</p></li>
				<li class="touch_mail"><a href="mailto:support@lorem.com">support@lorem.com</a></li>
				<li class="touch_site"><a href="javascript:void(0);">www.lorem.com</a></li>
				<li class="touch_adress"><p>3301 Lorem, Dolor Sit Amet St</p></li>
			</ul>
		</div>
		<div class="grid_9 omega">
			<div class="contact_form">  
				<div id="note"></div>
				<div id="fields">
					<form id="ajax-contact-form" action="">
						<input type="text" name="name" value="" placeholder="Name">
						<input type="text" name="subject" value="" placeholder="Phone">
						<input type="text" name="email" value="" placeholder="Email">
						<textarea name="message" id="message" placeholder="Message"></textarea>
						<div class="clear"></div>
						<input type="reset" class="contact_btn" value="Clear Form">
						<input type="submit" class="contact_btn send_btn" value="Send">
						<div class="clear"></div>
					</form>
				</div>
			</div>
		</div>
		<div class="clear"></div>
	
	</div>-->
	
	<div id="map_block"><iframe width="100%" height="772" frameborder="0" scrolling="no" marginheight="0" marginwidth="0" src="https://maps.google.com/maps?f=q&amp;source=s_q&amp;hl=en&amp;geocode=&amp;q=UCL,+Gower+Street,+London,+United+Kingdom&amp;aq=3&amp;oq=ucl&amp;sll=37.0625,-95.677068&amp;sspn=64.281297,135.263672&amp;ie=UTF8&amp;hq=UCL,+Gower+Street,+London,+United+Kingdom&amp;t=m&amp;ll=51.524371,-0.132963&amp;spn=0.006295,0.010159&amp;output=embed"></iframe></div>
	
</div>



<div id="footer">
	<div class="container clearfix">
		<div class="copyright">Hexes © 2014 | <a href="javascript:void(0);">Privacy Policy</a></div>
		<div id="back_top"><a class="button" title="" data-slide="1"></a></div>
	</div>
</div>




<div id="ui-datepicker-div" class="ui-datepicker ui-widget ui-widget-content ui-helper-clearfix ui-corner-all"></div></body></html>
