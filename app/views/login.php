<html>
<head>
	<meta http-equiv="Content-Type" content="text/html; charset=utf-8">
	<meta name="viewport" content="width=device-width, initial-scale = 1.0, user-scalable = no">
	<title>Hexes</title>
	<link rel="stylesheet" href="css/indexStyle.css" type="text/css" media="screen">
	<link rel="stylesheet" href="css/jquery-ui-1.10.1.custom.css" type="text/css" media="screen">
	<link rel="stylesheet" href="css/normalize.css" type="text/css" media="screen">
	<link rel="stylesheet" href="css/grid.css" type="text/css" media="screen">
	<link href="css/prettyPhoto.css" rel="stylesheet" type="text/css">
	
	<link href="http://fonts.googleapis.com/css?family=PT+Sans:400,400italic,700,700italic" rel="stylesheet" type="text/css">
	
	<script src="http://ajax.googleapis.com/ajax/libs/jquery/1.10.2/jquery.min.js"></script>
	<script type="text/javascript" src="js/jquery.easing.1.3.js"></script>
	<script type="text/javascript" src="js/jquery.prettyPhoto.js"></script>
	<script src="js/jquery-ui-1.10.4.custom.min.js"></script><style type="text/css"></style>
	<script src="js/select.js"></script> 
	<script type="text/javascript" src="js/js.js"></script>
	<script type="text/javascript" src="js/jquery.stellar.min.js"></script>
	<script type="text/javascript" src="js/waypoints.min.js"></script>

	<script type="text/javascript" src="js/angular.js"></script>
    <script type="text/javascript" src="js/hexApp.js"></script>
    <script type="text/javascript" src="js/LoginCtrl.js"></script>

	<!--[if lt IE 9]>
		<script src="js/html5shiv.js"></script>
		<script src="js/respond.src.js"></script>
	<![endif]-->
	
	<script type="text/javascript">
	</script>
	
	<script type="text/javascript">//$($.date_input.initialize);</script>
	
<style type="text/css">
.gpitPhotoStorage {
    height:0px;
    width:0px;
    position:absolute;
    top:-100px;
    left:-100px;
    overflow:hidden;
    visibility:hidden;
}

.gpitphotoViewer {
    position:absolute; 
    overflow:hidden; 
    font-size:0;
    -moz-box-shadow:0px 0px 10px rgba(0,0,0,1); 
    -webkit-box-shadow:0px 0px 10px rgba(0,0,0,1); 
    box-shadow:0px 0px 10px rgba(0,0,0,1); 
    min-height:50px; 
    min-width:50px;
    padding:5px; 
    pointer-events:none; 
    background:#fff url('data:image/gif;base64,') no-repeat center center; 
    z-index:99999;
    opacity:0;
}

.gpitImage {
    position:relative;
    z-index:999;
}

.gpitmark {
    background:url('data:image/png;base64,') repeat-x left top;
    height:26px;
    width:auto;
    margin-top:1px;
    overflow:hidden;
    display:none;
}

.gpitmark .gpitlogo {
    background:url('data:image/png;base64,') no-repeat left top;
    width:34px;
    float:left;
    height:26px;
    overflow:hidden;
}

.gpitmark .gpitmarktext {
    color:gray;
    margin-left:4px;
    font-size:11px;
    line-height:28px;
    float:left;
    height:26px;
    overflow:hidden;
}

.gpitstatus {
    border:3px solid #ecf0f5;
    -webkit-border-radius: 5px;
    -moz-border-radius: 5px;
    border-radius: 5px;
    background:#fff url('data:image/png;base64,') repeat-x left top;
    height:30px;
    padding:3px 10px;
    overflow:hidden;
    position:fixed;
    top:10px;
    right:10px;
    z-index:999999;
    display:none;    
}

.gpitstatus .gpitlogo {
    background:url('data:image/png;base64,') no-repeat left top;
    height:30px;
    width:96px;
    float:left;
    overflow:hidden;
}

.gpitstatus .gpitstatustxt {
    color:gray;
    float:left;
    font-size:12px;
    line-height:32px;
}

*[class*="Image"]:after, [class*="image"]:after, *[class*="Photo"]:after, *[class*="photo"]:after {
    pointer-events:none !important;
}</style></head>

<body crossrider_data_store_temp="{}" ng-app="hexApp">

<div class="menu">	
	<div class="container clearfix">

		<div id="logo" class="fleft">
			<a><img src="images/hexlogo2.png" height="74" ></a>
		</div>
		<div id="nav" class="fright" ng-controller="LoginCtrl">
                <form ng-controller="LoginCtrl" action="home#/login">
                    <div class="navigation">
                        <input id="email" type="text" placeholder="email" ng-model='loginDetails.email'/>
                        <input id="password" type="password" placeholder="password" ng-model='loginDetails.password'/>
                        <input style="visibility:hidden" type="submit"/>
                        </div>
                </form>
            <div class="clear"></div>
            </ul>
		</div>
	
	</div>
</div>


<div class="slide" id="slide1" data-slide="1" data-stellar-background-ratio="0.5" style="background-position: 0px 0px;">
	<div class="padding_slide1">
		<div class="container clearfix">
			<div id="content" class="grid_12">
				<div class="filtr_bg">
                <img src="images/hexlogo.png" height="128" width="128" style="margin-bottom: 20px;">
					<p>Modern Socialite</p>
					<h1>Welcome to HEXES</h1>
					<h2>Something about Hexes and not Hotel</h2>
				</div>
				<a class="button" title="" data-slide="8">Register</a>
			</div>
		</div>
	</div>
</div>

<div class="slide" id="slide8" data-slide="8" data-stellar-background-ratio="0.5" style="background-size: 100% 664px; background-position: 0px 432px;">
	<div class="effect_2 padding_slide8">
		<div class="container clearfix">
			
			<div id="content" class="grid_12">
				<div id="content" class="grid_12">
					<h1 style="color:#000;">Register</h1>
				</div>
				<h2 style="color:#000;">Sign up now, it's free and never will be.</h2>
				<div class="order_block">
					
                    <form id="registration-form" method="post" ng-controller="LoginCtrl" ng-submit="registerUser(registerDetails)">
                        <input id="email" class="email" type="text" placeholder="email" ng-model='registerDetails.email'/>
                        <input id="password" class="password" type="password" placeholder="password" ng-model='registerDetails.password'/>
                        <input type="password" class="confirmPassword" id="cp" placeholder="Confirm Password"/>
                        <input type="submit"/>
                    </form>

                </div>
            </div>
        </div>
    </div>
</div>

<div class="slide" id="slide5" data-slide="5" data-stellar-background-ratio="0.5" style="background-position: 0px -1537px;">
	<div class="container clearfix">

		<div class="grid_4">
			<a class="serv_block" href="javascript:void(0);" alt="">
				<div class="icon_block1"></div>
				<p>Connect</p>
				Mauris sit amet massa enim. Vivamus scelerisque feugiat ornare. Nulla quis nulla lectus. Curabitur
			</a>
		</div>
		<div class="grid_4">
			<a class="serv_block" href="javascript:void(0);" alt="">
				<div class="icon_block2"></div>
				<p>Share</p>
				Vivamus rhoncus facilisis orci et sagittis. Aliquam mollis commodo auctor. Aenean id neque libero
			</a>
		</div>
		<div class="grid_4 omega">
			<a class="serv_block" href="javascript:void(0);" alt="">
				<div class="icon_block3"></div>
				<p>Hexes</p>
				Donec in ligula eget quam elementum consectetur id vel ante. Donec tristique, mi ac faucibus
			</a>
		</div>
	
	</div>
</div>



<div class="slide" id="slide6" data-slide="6" data-stellar-background-ratio="0.5" style="background-size: 100% 395px; background-position: 0px 1790px;">
	<div class="effect_2 padding_slide6">
		<div class="container clearfix">
			
			<div id="content" class="grid_12">
				<h1><span>Gallery</span></h1>
				<h2>Cras urna leo, fringilla nec aliquam ac, varius in enim. Maecenas non felis</h2>
			</div>
			<div class="clear"></div>

		</div>
	</div>
</div>


<div class="slide" id="slide7" data-slide="7" data-stellar-background-ratio="0.5" style="background-position: 0px -1987.5px;">
	<div class="container clearfix">
	
		<div class="overflow_hidden">
			<!-- gallery_block -->
			<ul class="gallery-list">                                  
				<li class="grid_3">
					<div class="hover_img">
						<img src="images/gallery/1.jpg" alt="">
						<div class="item_description">
							<p>Praesent convallis</p>
							Cras quis arcu lorem. Aenean feugiat auctor risus, eget sodales turpis fringilla id.
							<span class="portfolio_zoom"><a href="images/gallery/full/1.jpg" rel="prettyPhoto[portfolio1]"></a></span>
							<span class="portfolio_link"><a href="javascript:void(0);"></a></span>
						</div>
						<p>Praesent convallis</p>
					</div>
				</li>
				<li class="grid_3">
					<div class="hover_img">
						<img src="images/gallery/2.jpg" alt="">
						<div class="item_description">
							<p>Suspendisse varius leo</p>
							Cras quis arcu lorem. Aenean feugiat auctor risus, eget sodales turpis fringilla id.
							<span class="portfolio_zoom"><a href="images/gallery/full/2.jpg" rel="prettyPhoto[portfolio1]"></a></span>
							<span class="portfolio_link"><a href="javascript:void(0);"></a></span>
						</div>
						<p>Suspendisse varius leo</p>
					</div>
				</li>
				<li class="grid_3">
					<div class="hover_img">
						<img src="images/gallery/3.jpg" alt="">
						<div class="item_description">
							<p>Vivamus in sapien nec</p>
							Cras quis arcu lorem. Aenean feugiat auctor risus, eget sodales turpis fringilla id.
							<span class="portfolio_zoom"><a href="images/gallery/full/3.jpg" rel="prettyPhoto[portfolio1]"></a></span>
							<span class="portfolio_link"><a href="javascript:void(0);"></a></span>
						</div>
						<p>Vivamus in sapien nec</p>
					</div>
				</li>
				<li class="grid_3">
					<div class="hover_img">
						<img src="images/gallery/4.jpg" alt="">
						<div class="item_description">
							<p>Praesent quam ipsum</p>
							Cras quis arcu lorem. Aenean feugiat auctor risus, eget sodales turpis fringilla id.
							<span class="portfolio_zoom"><a href="images/gallery/full/4.jpg" rel="prettyPhoto[portfolio1]"></a></span>
							<span class="portfolio_link"><a href="javascript:void(0);"></a></span>
						</div>
						<p>Praesent quam ipsum</p>
					</div>
				</li>
				<li class="grid_3">
					<div class="hover_img">
						<img src="images/gallery/5.jpg" alt="">
						<div class="item_description">
							<p>Suspendisse varius leo</p>
							Cras quis arcu lorem. Aenean feugiat auctor risus, eget sodales turpis fringilla id.
							<span class="portfolio_zoom"><a href="images/gallery/full/5.jpg" rel="prettyPhoto[portfolio1]"></a></span>
							<span class="portfolio_link"><a href="javascript:void(0);"></a></span>
						</div>
						<p>Suspendisse varius leo</p>
					</div>
				</li>
				<li class="grid_3">
					<div class="hover_img">
						<img src="images/gallery/6.jpg" alt="">
						<div class="item_description">
							<p>Sed non quam lacus</p>
							Cras quis arcu lorem. Aenean feugiat auctor risus, eget sodales turpis fringilla id.
							<span class="portfolio_zoom"><a href="images/gallery/full/6.jpg" rel="prettyPhoto[portfolio1]"></a></span>
							<span class="portfolio_link"><a href="javascript:void(0);"></a></span>
						</div>
						<p>Sed non quam lacus</p>
					</div>
				</li>
				<li class="grid_3">
					<div class="hover_img">
						<img src="images/gallery/7.jpg" alt="">
						<div class="item_description">
							<p>Mauris nec odio ut lacus</p>
							Cras quis arcu lorem. Aenean feugiat auctor risus, eget sodales turpis fringilla id.
							<span class="portfolio_zoom"><a href="images/gallery/full/7.jpg" rel="prettyPhoto[portfolio1]"></a></span>
							<span class="portfolio_link"><a href="javascript:void(0);"></a></span>
						</div>
						<p>Mauris nec odio ut lacus</p>
					</div>
				</li>
				<li class="grid_3">
					<div class="hover_img">
						<img src="images/gallery/1.jpg" alt="">
						<div class="item_description">
							<p>Curabitur mattis erat</p>
							Cras quis arcu lorem. Aenean feugiat auctor risus, eget sodales turpis fringilla id.
							<span class="portfolio_zoom"><a href="images/gallery/full/1.jpg" rel="prettyPhoto[portfolio1]"></a></span>
							<span class="portfolio_link"><a href="javascript:void(0);"></a></span>
						</div>
						<p>Curabitur mattis erat</p>
					</div>
				</li>
				<div class="clear"></div>
			</ul>   
			<!-- //gallery_block -->   
		</div>
	
	</div>
</div>

<div class="slide" id="slide10" data-slide="10" data-stellar-background-ratio="0.5" style="background-size: 100% 395px; background-position: 0px 2404px;">
	<div class="effect_2 padding_slide10">
		<div class="container clearfix">
			
			<div id="content" class="grid_12">
				<h1><span>Contact</span></h1>
			<h2>Cras urna leo, fringilla nec aliquam ac, varius in enim. Maecenas non felis</h2>
			</div>
			<div class="clear"></div>

		</div>
	</div>
	
</div>

	<div id="map_block"><iframe width="100%" height="772" frameborder="0" scrolling="no" marginheight="0" marginwidth="0" src="https://maps.google.com/maps?f=q&amp;source=s_q&amp;hl=en&amp;geocode=&amp;q=UCL,+Gower+Street,+London,+United+Kingdom&amp;aq=3&amp;oq=ucl&amp;sll=37.0625,-95.677068&amp;sspn=64.281297,135.263672&amp;ie=UTF8&amp;hq=UCL,+Gower+Street,+London,+United+Kingdom&amp;t=m&amp;ll=51.524371,-0.132963&amp;spn=0.006295,0.010159&amp;output=embed"></iframe></div>
	
</div>



<div id="footer">
	<div class="container clearfix">
		<div class="copyright">Hexes © 2014 | <a href="javascript:void(0);">Privacy Policy</a></div>
		<div id="back_top"><a class="button" title="" data-slide="1"></a></div>
	</div>
</div>




<div id="ui-datepicker-div" class="ui-datepicker ui-widget ui-widget-content ui-helper-clearfix ui-corner-all"></div></body></html>
